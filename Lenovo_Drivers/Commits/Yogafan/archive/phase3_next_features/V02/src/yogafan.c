...
static void apply_rllag_filter(struct yoga_fan_data *data, int idx, long raw_rpm)
{
	...
	if (data->last_sample[idx] == 0 || raw_dt_ms > MAX_SAMPLING) {
	...
	/* 5.Ensure minimal movement for small deltas */
	if (step == 0 && delta != 0)
		step = (delta > 0) ? 1 : -1;
			 ...
			 u32 attr, int channel, long *val)
{
	...
	/* Polling Protection: intercept fast queries to protect state variables */
